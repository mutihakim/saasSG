...
class TenantBankAccount extends Model
{
    ...
    {
        return $this->hasMany(FinanceWallet::class, 'real_account_id');
    }
    ...
    {
        return $this->hasOne(FinanceWallet::class, 'real_account_id')->where('is_system', true);
    }

    public function allocatedAmount(): float
    {
        $wallets = $this->relationLoaded('wallets')
            ? $this->getRelation('wallets')
            : ($this->relationLoaded('pockets')
                ? $this->getRelation('pockets')
                : $this->wallets()->get(['current_balance', 'is_system']));

        if (! $wallets instanceof Collection) {
            return 0.0;
        }

        return round((float) $wallets
            ->filter(fn (FinanceWallet $wallet) => ! (bool) $wallet->is_system)
            ->sum(fn (FinanceWallet $wallet) => (float) $wallet->current_balance), 2);
    }
