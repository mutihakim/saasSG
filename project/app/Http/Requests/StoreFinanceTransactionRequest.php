...
class StoreFinanceTransactionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'transfer_mode'      => ['nullable', 'in:account,member'],
            'from_account_id'    => ['nullable', 'string', 'size:26', 'required_if:type,transfer'],
            'to_account_id'      => ['nullable', 'string', 'size:26', 'required_if:type,transfer', 'different:from_account_id'],
            'budget_id'          => ['nullable', 'string', 'size:26', \Illuminate\Validation\Rule::exists('tenant_budgets', 'id')->where('tenant_id', $tenantId)],
            'bank_account_id'    => [\Illuminate\Validation\Rule::requiredIf(fn () => $this->input('type') !== 'transfer'), 'nullable', 'string', 'size:26', \Illuminate\Validation\Rule::exists('tenant_bank_accounts', 'id')->where('tenant_id', $tenantId)],
