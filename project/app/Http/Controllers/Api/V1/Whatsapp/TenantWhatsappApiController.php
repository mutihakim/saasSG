...
use App\Jobs\WhatsappDispatchJob;
use App\Models\Tenant\TenantMember;
use App\Models\Whatsapp\TenantWhatsappContact;
...
class TenantWhatsappApiController extends Controller
{
    ...
    public function sendToChat(Request $request, string $tenant, string $jid)
    {
        ...
        if ($isConnected && $this->serviceClient->isEnabled()) {
            if ($this->shouldQueueOutgoingDispatch()) {
                WhatsappDispatchJob::dispatch(
                    tenantId: (int) $tenant->id,
                    to: $chatJid,
    ...
    private function shouldQueueOutgoingDispatch(): bool
    {
        return (bool) config('whatsapp.dispatch_async', true)
