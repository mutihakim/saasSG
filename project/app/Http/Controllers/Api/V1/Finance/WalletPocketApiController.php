...
class WalletPocketApiController extends Controller
{
    ...
    public function index(Request $request, Tenant $tenant): JsonResponse
    {
        ...
        $walletsQuery = $detailLevel === WalletCashflowService::DETAIL_FULL
            ? $this->access->accessiblePocketsFullQuery($tenant, $member)
            : $this->access->accessiblePocketsSummaryQuery($tenant, $member);
        ...
        $wallets = $walletsQuery
            ...
            ->orderBy('name')
            ->get();
        $wallets = $this->cashflow->enrichPockets($tenant, $member, $wallets, detailLevel: $detailLevel);

        return response()->json(['ok' => true, 'data' => ['wallets' => $wallets]]);
    }

    public function show(Request $request, Tenant $tenant, FinanceWallet $wallet): JsonResponse
    {
        $member = $request->attributes->get('currentTenantMember');
        abort_unless($request->user()?->hasPermissionTo('finance.view'), 403);
        abort_if((int) $wallet->tenant_id !== (int) $tenant->id, 404);

        $record = $this->access->accessiblePocketsFullQuery($tenant, $member)
            ->whereKey($wallet->id)
            ->first();
        ...
        return response()->json(['ok' => true, 'data' => ['wallet' => $enriched]]);
    }
    ...
    public function store(Request $request, Tenant $tenant): JsonResponse
    {
        ...
        if (($data['scope'] ?? 'private') === 'shared') {
            ...
            $data['member_access'] = [];
            if ($data['type'] === 'shared') {
                $data['type'] = FinanceWalletService::DEFAULT_SHARED_TYPE;
            }
        } else {
            $data['member_access'] = [];
        }
        ...
        if ($wallet->scope === 'shared') {
            $this->wallets->syncWalletAccessFromAccount($account, $wallet);
        } else {
            $this->syncAccess($wallet, $tenant, [], null);
        }

        $fresh = $wallet->fresh(['ownerMember:id,full_name', 'memberAccess:id,full_name', 'realAccount:id,name,type,currency_code', 'defaultBudget:id,name,period_month,wallet_id,budget_key']);
        $this->activityLogs->log(
            ...
            ['scope' => $wallet->scope, 'is_system' => (bool) $wallet->is_system],
            null,
            (int) $wallet->row_version,
            $member
        );
    ...
    public function update(Request $request, Tenant $tenant, FinanceWallet $wallet): JsonResponse
    {
        $member = $request->attributes->get('currentTenantMember');
        abort_unless($request->user()?->hasPermissionTo('finance.update'), 403);
        abort_if((int) $wallet->tenant_id !== (int) $tenant->id, 404);
        abort_unless($this->access->canManagePocket($wallet, $member), 403);
        ...
        if ((int) $wallet->row_version !== (int) $data['row_version']) {
            return response()->json([
                'ok' => false,
        ...
        $account = $wallet->realAccount()->first();
        if (! $account) {
            return response()->json(['ok' => false, 'message' => 'Akun sumber wallet tidak ditemukan atau tidak bisa diakses.'], 422);
        }

        if (! $wallet->is_system && (string) $wallet->real_account_id !== (string) $data['real_account_id']) {
            return response()->json([
                'ok' => false,
        ...
        if (! $this->access->isPrivileged($member)) {
            if (! $this->access->canManagePocket($wallet, $member)) {
                abort(403);
            }

            if (! $this->access->canManageStructureSharing($member, $wallet->owner_member_id)) {
                $data['scope'] = $wallet->scope;
                $data['owner_member_id'] = $wallet->owner_member_id;
                $data['member_access'] = [];
            } else {
                $data['owner_member_id'] = $member?->id;
            }
        ...
        if ($wallet->is_system) {
            $data['name'] = FinanceWalletService::MAIN_POCKET_NAME;
            $data['type'] = 'personal';
        ...
        $before = $this->activityLogs->snapshot($wallet->load(['memberAccess:id', 'realAccount:id', 'defaultBudget:id']));
        $beforeVersion = (int) $wallet->row_version;

        $wallet->update([
            'real_account_id' => $account->id,
            'owner_member_id' => $wallet->is_system ? $account->owner_member_id : ($data['owner_member_id'] ?? null),
            'name' => $wallet->is_system ? FinanceWalletService::MAIN_POCKET_NAME : $data['name'],
            'slug' => Str::slug($wallet->is_system ? FinanceWalletService::MAIN_POCKET_NAME : $data['name']),
            'type' => $wallet->is_system ? 'main' : $data['type'],
            'purpose_type' => $data['purpose_type'],
            'background_color' => $data['background_color'] ?? null,
            'scope' => $wallet->is_system ? $account->scope : $data['scope'],
            'currency_code' => $account->currency_code,
            'icon_key' => $data['icon_key'] ?? null,
            'default_budget_id' => $data['purpose_type'] === 'spending' ? $defaultBudget?->id : null,
            'default_budget_key' => $data['purpose_type'] === 'spending' ? ($defaultBudget?->budget_key ?? ($data['default_budget_key'] ?? null)) : null,
            'budget_lock_enabled' => $data['purpose_type'] === 'spending' ? (bool) ($data['budget_lock_enabled'] ?? false) : false,
            'notes' => $wallet->is_system ? $wallet->notes : ($data['notes'] ?? null),
            'is_active' => $wallet->is_system ? (bool) $account->is_active : ($data['is_active'] ?? $wallet->is_active),
            'row_version' => $wallet->row_version + 1,
        ]);

        $finalScope = $wallet->is_system ? $account->scope : ($data['scope'] ?? $wallet->scope);
    ...
    public function destroy(Request $request, Tenant $tenant, FinanceWallet $wallet): JsonResponse
    {
        $member = $request->attributes->get('currentTenantMember');
        ...
        $before = $this->activityLogs->snapshot($wallet->load(['ownerMember:id,full_name', 'memberAccess:id,full_name', 'realAccount:id,name,type,currency_code']));
        $beforeVersion = (int) $wallet->row_version;
        $wallet->delete();
    ...
    private function syncAccess(FinanceWallet $wallet, Tenant $tenant, array $memberAccess, ?int $actorMemberId): void
    {
        ...
        if ($wallet->owner_member_id) {
            $sync[(int) $wallet->owner_member_id] = ['can_view' => true, 'can_use' => true, 'can_manage' => true];
        }
        ...
        $wallet->memberAccess()->sync(array_intersect_key($sync, array_flip($validIds)));
    }
    ...
    private function nextReferenceCode(Tenant $tenant): string
    {
        ...
        } while (FinanceWallet::query()->where('tenant_id', $tenant->id)->where('reference_code', $reference)->exists());
