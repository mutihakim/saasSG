...
class WalletSavingsGoalApiController extends Controller
{
    ...
    public function index(Request $request, Tenant $tenant): JsonResponse
    {
        ...
        $accessiblePocketIds = $this->access->accessiblePocketsQuery($tenant, $member)->pluck('finance_wallets.id');
    ...
    public function store(Request $request, Tenant $tenant): JsonResponse
    {
        ...
        $data = $request->validate([
            'wallet_id' => ['required', 'string', 'size:26', Rule::exists('finance_wallets', 'id')->where('tenant_id', $tenant->id)],
        ...
        $pocket = $this->access->usablePocketsQuery($tenant, $member)->whereKey($data['wallet_id'])->first();
        if (! $pocket) {
    ...
    public function update(Request $request, Tenant $tenant, FinanceSavingsGoal $goal): JsonResponse
    {
        ...
        if ((string) $goal->wallet_id !== (string) $pocket->id && (float) $goal->current_amount > 0) {
    ...
    public function fund(Request $request, Tenant $tenant, FinanceSavingsGoal $goal): JsonResponse
    {
        ...
        $data = $request->validate([
            'source_wallet_id' => ['required', 'string', 'size:26', Rule::exists('finance_wallets', 'id')->where('tenant_id', $tenant->id)],
        ...
        $sourcePocket = $this->resolver->resolveUsablePocket($tenant, $member, $data['source_wallet_id']);
        ...
        try {
            $result = DB::transaction(function () use ($request, $tenant, $member, $targetGoal, $sourcePocket, $targetPocket, $data) {
                ...
                $reservedByPocket = FinanceSavingsGoal::query()
                    ->whereIn('wallet_id', $lockedPockets->keys())
                    ->selectRaw('wallet_id, COALESCE(SUM(current_amount), 0) as reserved_total')
                    ->groupBy('wallet_id')
                    ->get()
                    ->keyBy('wallet_id');
                ...
                if ((string) $lockedSourcePocket->id !== (string) $lockedTargetPocket->id) {
                    ...
                    $sourceTx = FinanceTransaction::create($this->payloads->transactionPayload(
                        ...
                        data: [
                            ...
                            'wallet_id' => $lockedSourcePocket->id,
                            'budget_id' => null,
                    ...
                    $targetTx = FinanceTransaction::create($this->payloads->transactionPayload(
                        ...
                        data: [
                            ...
                            'wallet_id' => $lockedTargetPocket->id,
                            'budget_id' => null,
                            'budget_status' => 'unbudgeted',
                            'budget_delta' => 0,
            ...
            $this->writeGoalActivityLog($request, $tenant, $member, 'finance.goal.funded', $result, null, $this->goalSnapshot($result), [
                'source_wallet_id' => $sourcePocket->id,
    ...
    public function spend(Request $request, Tenant $tenant, FinanceSavingsGoal $goal): JsonResponse
    {
        ...
        try {
            $result = DB::transaction(function () use ($request, $tenant, $member, $goal, $data) {
                ...
                $lockedPocket = FinanceWallet::query()->whereKey($lockedGoal->wallet_id)->lockForUpdate()->first();
    ...
    {
        $pocket = $goal->pocket ?: FinanceWallet::query()->find($goal->wallet_id);
