...
class FinanceAccountApiController extends Controller
{
    ...
    public function index(Request $request, Tenant $tenant): JsonResponse
    {
        ...
        $accountsQuery = $detailLevel === WalletCashflowService::DETAIL_FULL
            ? $this->access->accessibleAccountsFullQuery($tenant, $member)
            : $this->access->accessibleAccountsSummaryQuery($tenant, $member);
    ...
    public function show(Request $request, Tenant $tenant, TenantBankAccount $account): JsonResponse
    {
        ...
        $record = $this->access->accessibleAccountsFullQuery($tenant, $member)
            ->whereKey($account->id)
            ->first();
    ...
    public function update(Request $request, Tenant $tenant, TenantBankAccount $account): JsonResponse
    {
        ...
        $this->wallets->ensureMainWallet($account);
        $this->wallets->applyOpeningBalanceDelta($account, round($nextOpeningBalance - $previousOpeningBalance, 2));
        $this->wallets->syncInheritedWalletsFromAccount($account->fresh(['memberAccess:id']));
