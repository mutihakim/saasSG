...
class MonthlyReviewService
{
    ...
    public function buildStatus(Tenant $tenant, ?TenantMember $member): array
    {
        ...
        $reviews = FinanceMonthReview::query()
            ->forTenant($tenant->id)
            ->whereIn('period_month', $this->eligibleMonths($tenant, $member))
            ->get()
            ->keyBy('period_month');

        $eligibleMonths = collect($this->eligibleMonths($tenant, $member))
    ...
    public function isPlanningBlockedForPeriod(Tenant $tenant, string $periodMonth): bool
    {
        ...
        $previousMonth = now()->startOfMonth()->subMonthNoOverflow()->format('Y-m');
        $hasPreviousMonthActivity = FinanceTransaction::query()
            ->where('tenant_id', $tenant->id)
            ->whereRaw("substr(cast(transaction_date as text), 1, 7) = ?", [$previousMonth])
            ->exists()
            || TenantBudget::query()
                ->where('tenant_id', $tenant->id)
