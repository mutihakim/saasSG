...
type Props = {
    show: boolean;
    onHide: () => void;
    onSave: (values: GoalFormState) => Promise<void>;
    saving: boolean;
...
const GoalFormWizard = ({ show, onHide, onSave, saving, goal, wallets }: Props) => {
    ...
    const [formData, setFormData] = useState<GoalFormState>({
        pocket_id: goal?.pocket_id || wallets.find((wallet) => !wallet.is_system)?.id || wallets[0]?.id || "",
        name: goal?.name || "",
    ...
    const selectedWallet = useMemo(
        () => wallets.find((wallet) => String(wallet.id) === String(formData.pocket_id)) ?? null,
        [formData.pocket_id, wallets],
    );

    const remainingAmount = Math.max(Number(formData.target_amount || 0) - Number(goal?.current_amount || 0), 0);

    const updateField = (field: keyof GoalFormState, value: GoalFormState[keyof GoalFormState]) => {
    ...
    const validateStep1 = () => {
        ...
        if (!formData.pocket_id) {
            nextErrors.pocket_id = "Wallet induk goal wajib dipilih";
        }
                                ...
                                <Col md={7}>
                                    <Form.Group>
                                        ...
                                        <Form.Select
                                            ...
                                            onChange={(event) => updateField("pocket_id", event.target.value)}
                                            isInvalid={Boolean(errors.pocket_id)}
                                        >
                                        ...
                                        <Form.Control.Feedback type="invalid">{errors.pocket_id}</Form.Control.Feedback>
                                    </Form.Group>
                                </Col>
                                <Col md={5}>
                                    <Form.Group>
