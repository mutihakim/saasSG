...
const WalletAccountModal = ({
    ...
    useEffect(() => {
        ...
        if (source) {
            setFormData({
                name: source.name ?? "",
                scope: canManageShared ? source.scope ?? "private" : "private",
                ...
                member_access_ids: canManageShared ? (source.member_access || []).map((member: any) => member.id) : [],
                opening_balance: String(source.opening_balance ?? source.current_balance ?? "0"),
                notes: source.notes ?? "",
    ...
    }, [show, account, seedAccount, currencies, activeMemberId, canManageShared]);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await axios({
                ...
                url: isEdit
                    ? tenantRoute.apiTo(`/wallet/accounts/${account.id}`)
                    : tenantRoute.apiTo("/wallet/accounts"),
                data: {
                    ...formData,
                ...
                <Modal.Body>
                    <Row className="g-3">
                        <Col md={12}>
                            ...
                            <Form.Control value={formData.name} onChange={(e) => setFormData((prev) => ({ ...prev, name: e.target.value }))} required />
                        </Col>
                        <Col md={6}>
                        ...
                        <Col md={12}>
                            ...
                            <Form.Control as="textarea" rows={3} value={formData.notes} onChange={(e) => setFormData((prev) => ({ ...prev, notes: e.target.value }))} />
                        </Col>
                    </Row>
                </Modal.Body>
                <Modal.Footer>
                    {isEdit && canDelete && onDelete && (
                        <Button variant="outline-danger" onClick={onDelete} disabled={loading}>
