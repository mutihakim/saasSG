...
import { buildCategoryBreakdown, buildCategoryChartOptions } from "./finance-derived/categoryMetrics";
...
export const useFinanceDerivedMetrics = ({
    ...
    const categoryBreakdown = useMemo<FinanceCategoryBreakdownItem[]>(() => {
        return buildCategoryBreakdown({
            transactions,
            statsMetric,
            uncategorizedLabel: t("finance.shared.uncategorized"),
        });
    }, [statsMetric, t, transactions]);

    const categoryChartOptions: ApexCharts.ApexOptions = useMemo(
        () => buildCategoryChartOptions(categoryBreakdown),
        [categoryBreakdown],
    );
