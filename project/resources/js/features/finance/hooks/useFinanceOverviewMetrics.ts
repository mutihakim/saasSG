...
const useFinanceOverviewMetrics = ({
    activeTab,
    filteredAccounts,
    filteredWallets,
    wishes,
    ...
    const groupedWallets = useMemo<FinanceGroupedAccountPockets[]>(() => filteredAccounts.map((account) => ({
        account,
    ...
    const metrics = useMemo<FinanceMetricsItem[]>(() => {
        if (!summary) {
            return [];
        }
...
export default useFinanceOverviewMetrics;
