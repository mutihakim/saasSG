...
import {
    FinanceMonthlyReviewBudgetDraft,
    FinanceMonthlyReviewPreview,
    FinanceMonthlyReviewStatus,
    FinanceMonthlyReviewSweepDraft,
} from "../types";
...
const MonthlyReviewWizard = ({ show, onHide, monthlyReview, syncAll }: Props) => {
    ...
    const [preview, setPreview] = useState<FinanceMonthlyReviewPreview | null>(null);
    const [sweepDrafts, setSweepDrafts] = useState<FinanceMonthlyReviewSweepDraft[]>([]);
    const [budgetDrafts, setBudgetDrafts] = useState<FinanceMonthlyReviewBudgetDraft[]>([]);
    ...
    const loadPreview = useCallback(async (periodMonth: string, method: "copy_last_month" | "average_3_months" | "zero_based") => {
        if (!periodMonth) {
            return;
        }
        ...
        try {
            ...
            const payload = response.data?.data?.preview as FinanceMonthlyReviewPreview;
            setPreview(payload);
    ...
    }, [show, selectedMonth, budgetMethod, loadPreview]);
                ...
                {!loading && preview && activeStep === 2 && (
                    <div className="d-flex flex-column gap-3">
                        {preview.wallets.filter((wallet) => Number(wallet.ending_balance) > 0 && !wallet.is_system).map((wallet) => {
                            const draft = sweepDrafts.find((item) => item.source_wallet_id === wallet.id);
                                            ...
                                            <Col md={4}>
                                                <Form.Group>
                                                    ...
                                                    <Form.Select
                                                        ...
                                                        onChange={(event) => setSweepDrafts((prev) => prev.map((item) => item.source_wallet_id === wallet.id ? { ...item, action: event.target.value as FinanceMonthlyReviewSweepDraft["action"] } : item))}
                                                    >
                                            ...
                                            <Col md={3}>
                                                <Form.Group>
                                                    ...
                                                    <Form.Control
                                                        ...
                                                        onChange={(event) => setSweepDrafts((prev) => prev.map((item) => item.source_wallet_id === wallet.id ? { ...item, amount: event.target.value } : item))}
                                                    />
                                                </Form.Group>
                                            </Col>
                                            <Col md={5}>
                                                {draft?.action === "sweep_to_wallet" && (
                                                    <Form.Group>
                                                        ...
                                                        <Form.Select
                                                            ...
                                                            onChange={(event) => setSweepDrafts((prev) => prev.map((item) => item.source_wallet_id === wallet.id ? { ...item, target_wallet_id: event.target.value } : item))}
                                                        >
                                                ...
                                                {draft?.action === "sweep_to_goal" && (
                                                    <Form.Group>
                                                        ...
                                                        <Form.Select
                                                            value={draft?.goal_id || ""}
                                                            onChange={(event) => setSweepDrafts((prev) => prev.map((item) => item.source_wallet_id === wallet.id ? { ...item, goal_id: event.target.value } : item))}
                                                        >
