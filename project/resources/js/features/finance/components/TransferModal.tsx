...
const TransferModal = ({ show, onClose, onSuccess, accounts, pockets, destinationPockets, members, activeMemberId }: TransferModalProps) => {
    ...
    const toPocketOptions = useMemo(() => {
        ...
        return filtered.filter((option) => option.currencyCode === formData.currency_code && String(option.value) !== String(formData.from_wallet_id));
    }, [destinationPocketOptions, formData.currency_code, formData.from_wallet_id, formData.recipient_member_id, formData.transfer_mode]);

    const selectedFromPocket = useMemo(() => visiblePockets.find((pocket) => String(pocket.id) === String(formData.from_wallet_id)) ?? null, [formData.from_wallet_id, visiblePockets]);
    const selectedToPocket = useMemo(() => destinationPockets.find((pocket) => String(pocket.id) === String(formData.to_wallet_id)) ?? null, [destinationPockets, formData.to_wallet_id]);
    ...
    useEffect(() => {
        ...
        setFormData((prev) => ({
            ...
            from_wallet_id: prev.from_wallet_id || (visiblePockets[0]?.id ? String(visiblePockets[0].id) : ""),
                    ...
                    <Row className="g-3">
                            ...
                            <div className="d-flex gap-2 mt-1">
                                {[
                                    ...
                                    <button
                                        ...
                                        onChange={() => setFormData((prev) => ({ ...prev, transfer_mode: mode.value, recipient_member_id: "", to_wallet_id: "" }))}
                                        onClick={() => setFormData((prev) => ({ ...prev, transfer_mode: mode.value, recipient_member_id: "", to_wallet_id: "" }))}
                                    >
                                        {mode.label}
                                    </button>
                                ))}
                        ...
                        {formData.transfer_mode === "member" && (
                            <Col xs={12}>
                                ...
                                <Form.Select
                                    ...
                                    onChange={(e) => setFormData((prev) => ({ ...prev, recipient_member_id: e.target.value, to_wallet_id: "" }))}
                                >
                        ...
                        <Col xs={12}>
                            ...
                            <Form.Select
                                ...
                                onChange={(e) => setFormData((prev) => ({
                                    ...
                                    to_wallet_id: prev.to_wallet_id === e.target.value ? "" : prev.to_wallet_id,
                                }))}
                            >
                        ...
                        <Col xs={12}>
                            ...
                            <Form.Select
                                ...
                                onChange={(e) => setFormData((prev) => ({ ...prev, to_wallet_id: e.target.value }))}
                            >
