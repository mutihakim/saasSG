...
const TransactionDetailSheet = ({
    ...
    const [prevId, setPrevId] = useState<string | number | undefined>(undefined);

    if (!show && prevId !== undefined) {
        setPrevId(undefined);
    }

    if (show && transaction?.id !== prevId) {
    ...
    const transactionWallet = transaction.wallet || transaction.pocket || null;
    const sourceAccount = transaction.transfer_direction === "out" ? transaction.bank_account : pairedTransaction?.bank_account;
    const sourceWallet = transaction.transfer_direction === "out"
        ? transactionWallet
        : (pairedTransaction?.wallet || pairedTransaction?.pocket || null);
    const targetAccount = transaction.transfer_direction === "in" ? transaction.bank_account : pairedTransaction?.bank_account;
    const targetWallet = transaction.transfer_direction === "in"
        ? transactionWallet
        : (pairedTransaction?.wallet || pairedTransaction?.pocket || null);
    ...
    const budgetMode = transactionWallet?.budget_lock_enabled ? "Locked" : transactionWallet?.default_budget_key || transactionWallet?.default_budget_id ? "Recommended" : "Flexible";
    const activeBudget = transaction.budget || transactionWallet?.default_budget || null;
                        ...
                        <div className="card-body text-center p-4">
                            ...
                            <div className="d-flex justify-content-center flex-wrap gap-2 mt-4">
                                ...
                                <div className="badge rounded-pill px-3 py-2 d-flex align-items-center gap-1 shadow-sm text-white" style={{ background: '#3b82f6' }}>
                                    <i className="ri-wallet-3-line"></i> {transaction.type === "transfer" ? (sourceWallet?.name || "-") : (transactionWallet?.name || "Wallet")}
                                </div>
                                {isGrouped && (
                                ...
                                {transaction.type === "transfer" ? (
                                    <>
                                        ...
                                        <div className="border border-success rounded-pill px-3 py-2 d-flex justify-content-between text-success fw-medium small" style={{ background: 'rgba(34, 197, 94, 0.05)' }}>
                                            ...
                                            <span className="d-flex align-items-center gap-1"><i className="ri-wallet-3-line"></i> {sourceWallet?.name || "-"}</span>
                                        </div>
                                        <div className="border border-success rounded-pill px-3 py-2 d-flex justify-content-between text-success fw-medium small" style={{ background: 'rgba(34, 197, 94, 0.05)' }}>
                                        ...
                                        <div className="border border-success rounded-pill px-3 py-2 d-flex justify-content-between text-success fw-medium small" style={{ background: 'rgba(34, 197, 94, 0.05)' }}>
                                            ...
                                            <span className="d-flex align-items-center gap-1"><i className="ri-wallet-3-line"></i> {targetWallet?.name || "-"}</span>
                                        </div>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <div className="border border-success rounded-pill px-3 py-2 d-flex justify-content-between text-success fw-medium small" style={{ background: 'rgba(34, 197, 94, 0.05)' }}>
                                            ...
                                            <span className="d-flex align-items-center gap-1"><i className="ri-wallet-3-line"></i> {transactionWallet?.name || "-"}</span>
                                        </div>
                                    </>
                                )}
                                <div className="border border-success rounded-pill px-3 py-2 d-flex justify-content-between text-success fw-medium small" style={{ background: 'rgba(34, 197, 94, 0.05)' }}>
