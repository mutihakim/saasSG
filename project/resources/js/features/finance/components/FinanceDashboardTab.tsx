...
import { FinanceMonthlyReviewStatus, FinanceWish, FinanceWalletSummary } from "../types";
...
const FinanceDashboardTab = ({ 
    ...
    const suggestedMonth = monthlyReview?.suggested_period_month || monthlyReview?.previous_month || null;
    const planningBlocked = Boolean(monthlyReview?.planning_blocked);
    const isClosed = monthlyReview?.previous_month_status === "closed";
    const canOpenMonthlyReview = Boolean(monthlyReview?.suggested_period_month);
    const hasMonthlyReview = Boolean(monthlyReview);

    if (!summary) {
        return (
            <div className="text-center py-5">
                <div className="text-muted small mt-2">{t("wallet.dashboard.no_summary", { defaultValue: "Tidak ada ringkasan tersedia." })}</div>
            </div>
        );
    }
                        ...
                        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                            <div>
                                ...
                                <div className="fw-bold mt-1" style={{ fontSize: "1.05rem" }}>
                                    {suggestedMonth || t("wallet.dashboard.monthly_review_unavailable_title", { defaultValue: "Belum tersedia" })}
                                </div>
                                <div className="small text-muted mt-1">
                                    {hasMonthlyReview
                                        ...
                                        : t("wallet.dashboard.monthly_review_unavailable_hint", { defaultValue: "Monthly review belum tersedia untuk tenant ini." })}
                                </div>
                            </div>
                            <div className="d-flex align-items-center gap-2">
                                <Badge bg={hasMonthlyReview ? (planningBlocked ? "warning" : "success") : "secondary"}>
                                    {hasMonthlyReview
                                        ? isClosed
                                        ...
                                        : t("wallet.dashboard.monthly_review_unavailable_badge", { defaultValue: "Unavailable" })}
                                </Badge>
                                <button
                                    type="button"
                                    className={`btn rounded-pill px-3 ${isClosed ? "btn-outline-primary" : "btn-primary"}`}
                                    onClick={onOpenMonthlyReview}
                                    disabled={!canOpenMonthlyReview}
                                >
                                    {isClosed
                                        ...
                                        : hasMonthlyReview
                                            ...
                                            : t("wallet.dashboard.monthly_review_unavailable_button", { defaultValue: "Not Available" })}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <Card className="border-0 shadow-sm rounded-4 h-100">
                        <Card.Body className="p-3">
                            ...
                            {assetSeries.length > 0 ? (
                                <LazyApexChart options={assetOptions} series={assetSeries} type="donut" height={280} />
                            ) : (
                ...
                <Col md={4}>
                    <Card className="border-0 shadow-sm rounded-4 h-100">
                        <Card.Body className="p-3">
                            ...
                            <LazyApexChart options={debtOptions} series={[Number(summary.debtRatio || 0)]} type="radialBar" height={250} />
