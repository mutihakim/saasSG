...
import { FinanceBudget, FinanceCategory, FinancePaymentMethodOption, FinanceWallet, FinanceSavingsGoal } from "../types";
import { FinanceGoalFundFormState, FinanceGoalSpendFormState } from "../types";
...
type FundProps = {
    ...
    form: FinanceGoalFundFormState;
    setForm: React.Dispatch<React.SetStateAction<FinanceGoalFundFormState>>;
...
type SpendProps = {
    ...
    form: FinanceGoalSpendFormState;
    setForm: React.Dispatch<React.SetStateAction<FinanceGoalSpendFormState>>;
    onSubmit: () => Promise<void>;
    loading: boolean;
};
...
const GoalActionModal = (props: Props) => {
    ...
    const wallet = useMemo(
        () => props.wallets.find((item) => String(item.id) === String(props.goal?.wallet_id)) ?? null,
        [props.goal?.wallet_id, props.wallets],
    );
                ...
                {props.mode === "fund" ? (
                    <Row className="g-3">
                        <Col xs={12}>
                            ...
                            <Form.Select value={props.form.source_wallet_id} onChange={(event) => props.setForm((prev) => ({ ...prev, source_wallet_id: event.target.value }))}>
